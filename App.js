...
import{View,Text,TextInput,TouchableOpacity,ScrollView,StyleSheet,Switch,StatusBar,Alert,Modal,AppState,NativeModules,NativeEventEmitter,Platform}from'react-native';
...
const cardConnected=useRef(false);const readerRelay=useRef(false);const isoDepRef=useRef(null);
const urlRef=useRef('wss://nfctuneless.serveousercontent.com');
const roleRef=useRef('both');
...
setTimeout(()=>{if(urlRef.current)connect(urlRef.current);},1000);
}}});
return()=>sub.remove();
},[]);
useEffect(()=>{
...
addEv({type:'HCE',src:'POS',data:'POS: '+apdu?.slice(0,16)});
...
}else{HceModule.deliverResponse(requestId,'6F00');}
}else{HceModule.deliverResponse(requestId,'6F00');}
});
return()=>sub.remove();
},[clients]);
...
if(!nfcOk||!nfcOn){Alert.alert('NFC','Activati NFC!');setAutoScan(false);return;}
...
const u=connectUrl||urlRef.current;
...
setShowConnectPin(false);setPinInput('');
},[pinInput,connect]);
...
if(m.pin)setServerPin(m.pin);break;
...
const relayToCard=useCallback(async(m)=>{
try{
if(!isoDepRef.current){ws.current?.send(JSON.stringify({type:'APDU_RELAY_RESPONSE',requestId:m.requestId,apdu:'6F00'}));return;}
...
},[hMode,hCust,addEv,relayToCard]);
const startReaderRelay=useCallback(async()=>{
if(!nfcOk||!nfcOn){Alert.alert('NFC','Activati NFC!');return;}
try{
await NfcManager.requestTechnology([NfcTech.IsoDep]);
const tag=await NfcManager.getTag();
isoDepRef.current=tag;readerRelay.current=true;
addEv({type:'HCE',src:'CARD',data:'Card conectat! Relay activ.'});
setHLog(p=>[{cmd:'CARD CONECTAT',rsp:'RELAY ACTIV',ts:Date.now()},...p]);
Alert.alert('Card Conectat!','Relay activ - comenzile APDU merg la card');
}catch(e){readerRelay.current=false;isoDepRef.current=null;Alert.alert('Eroare',e.message);}
},[nfcOk,nfcOn,addEv]);
const stopReaderRelay=useCallback(()=>{
...
<View style={s.card}><Text style={s.lbl}>CONECTARE MANUALA</Text>
...
<View><Text style={{fontFamily:'monospace',fontSize:14,color:C.c1,fontWeight:'bold'}}>HCE EMULATOR</Text><Text style={{fontFamily:'monospace',fontSize:10,marginTop:2,color:hOn?C.c3:C.t3}}>{hOn?'ACTIV':'INACTIV'}</Text></View>
<Switch value={hOn} onValueChange={setHOn} trackColor={{false:C.b2,true:'rgba(57,255,20,0.3)'}} thumbColor={hOn?C.c3:C.t3}/>
</View>
<View style={{marginTop:12,marginBottom:10}}>
<TouchableOpacity style={{borderWidth:1,borderColor:readerRelay.current?C.c3:C.c2,borderRadius:6,padding:12,alignItems:'center',marginBottom:6,backgroundColor:readerRelay.current?'rgba(57,255,20,0.08)':'rgba(255,102,0,0.08)'}} onPress={readerRelay.current?stopReaderRelay:startReaderRelay}>
<Text style={{fontFamily:'monospace',fontSize:12,color:readerRelay.current?C.c3:C.c2,letterSpacing:2}}>{readerRelay.current?'OPRESTE CARD RELAY':'CONECTEAZA CARD FIZIC'}</Text>
</TouchableOpacity>
<Text style={{fontFamily:'monospace',fontSize:9,color:C.t3,textAlign:'center'}}>{readerRelay.current?'Card conectat - relay activ':'Apropie cardul fizic dupa ce apesi'}</Text>
</View>
<Text style={s.lbl}>MOD RASPUNS</Text>
<View style={{flexDirection:'row',gap:6}}>{['Auto','Relay WS','Custom'].map((m,i)=>(<TouchableOpacity key={m} style={{flex:1,borderWidth:1,borderColor:hMode===i?C.c1:C.b2,borderRadius:4,padding:8,alignItems:'center',backgroundColor:hMode===i?'rgba(0,212,255,0.08)':'transparent'}} onPress={()=>setHMode(i)}><Text style={{fontFamily:'monospace',fontSize:9,color:hMode===i?C.c1:C.t3,textAlign:'center'}}>{m}</Text></TouchableOpacity>))}</View>
...
<TextInput style={{fontFamily:'monospace',fontSize:32,backgroundColor:C.bg0,borderWidth:2,borderColor:C.c5,color:C.t1,padding:14,borderRadius:8,textAlign:'center',letterSpacing:8,marginBottom:16}} value={pinInput} onChangeText={t=>setPinInput(t.replace(/\D/g,'').slice(0,6))} placeholder="000000" placeholderTextColor={C.t3} keyboardType="numeric" maxLength={6}/>
